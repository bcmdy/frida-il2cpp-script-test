import path, { normalize } from '@frida/crosspath';
 ...
 */
function resolvePath({ id, parent, prioritizedExtensions = ["", ".js", ".mjs", ".cjs", ".jsx", ".ts", ".mts", ".cts", ".tsx", ".json"], fileSystem, resolveModule, resolveCache }) {
    var _a, _b;
    ...
    try {
        const resolveResult = resolveModule(id, parent !== null && parent !== void 0 ? parent : null);
        // Add it to the cache
...
function walkThroughFillerNodes(expression, typescript) {
    ...
    if (typescript.isParenthesizedExpression(expression) ||
        ...
        ((_a = typescript.isTypeAssertionExpression) === null || _a === void 0 ? void 0 : _a.call(typescript, expression)) ||
        ((_b = typescript.isTypeAssertion) === null || _b === void 0 ? void 0 : _b.call(typescript, expression)) ||
            ...
            if (typescript.isIdentifier(obj) && obj.text === "module" &&
                ...
                for (const element of descriptor.properties) {
                    ...
                    if (name.text === "get") {
                        ...
                        context.addTrailingStatements(factory.createExportAssignment(undefined, undefined, false, factory.createCallExpression(initializer, [], [])));
                        return undefined;
                    }
                }
            }
    ...
    if (moduleExports == null || moduleExports.namedExports.size === 0 || (expressionStatementParent != null && !moduleExports.hasDefaultExport)) {
        ...
        if (expressionStatementParent != null) {
            ...
            if (!context.isModuleSpecifierImportedWithoutLocals(moduleSpecifier)) {
                context.addImport(factory.createImportDeclaration(undefined, undefined, undefined, factory.createStringLiteral(transformedModuleSpecifier), maybeGenerateAssertClause(context, transformedModuleSpecifier, moduleExports === null || moduleExports === void 0 ? void 0 : moduleExports.assert)), moduleSpecifier);
            }
        ...
        // Otherwise, we need to give the module a name and replace the CallExpression with an identifier for it
            ...
            if (context.hasLocalForDefaultImportFromModule(moduleSpecifier)) {
                ...
                context.addImport(factory.createImportDeclaration(undefined, undefined, importClause, factory.createStringLiteral(transformedModuleSpecifier), maybeGenerateAssertClause(context, transformedModuleSpecifier, moduleExports === null || moduleExports === void 0 ? void 0 : moduleExports.assert)), moduleSpecifier);
        ...
        if (rightValue != null) {
            ...
            if (!moduleExports.namedExports.has(rightValue)) {
                ...
                // rather than generating a new unnecessary import
                    ...
                    context.addImport(factory.createImportDeclaration(undefined, undefined, moduleExports.hasDefaultExport
...
function ensureNodeHasExportModifier(node, context) {
    ...
    const newModifiers = [context.factory.createModifier(typescript.SyntaxKind.ExportKeyword), ...existingModifierKinds.map(kind => factory.createModifier(kind))];
    if (typescript.isFunctionDeclaration(node)) {
        return context.factory.updateFunctionDeclaration(node, node.decorators, newModifiers, node.asteriskToken, node.name, node.typeParameters, node.parameters, node.type, node.body);
    }
    else if (typescript.isFunctionExpression(node)) {
        return context.factory.updateFunctionExpression(node, newModifiers, node.asteriskToken, node.name, node.typeParameters, node.parameters, node.type, node.body);
    }
    else if (typescript.isClassDeclaration(node)) {
        return context.factory.updateClassDeclaration(node, node.decorators, newModifiers, node.name, node.typeParameters, node.heritageClauses, node.members);
    }
    else if (typescript.isClassExpression(node)) {
        return context.factory.updateClassExpression(node, node.decorators, newModifiers, node.name, node.typeParameters, node.heritageClauses, node.members);
    }
    else if (typescript.isVariableStatement(node)) {
        return context.factory.updateVariableStatement(node, newModifiers, node.declarationList);
    }
    else if (typescript.isEnumDeclaration(node)) {
        return context.factory.updateEnumDeclaration(node, node.decorators, newModifiers, node.name, node.members);
    }
    else if (typescript.isInterfaceDeclaration(node)) {
        return context.factory.updateInterfaceDeclaration(node, node.decorators, newModifiers, node.name, node.typeParameters, node.heritageClauses, node.members);
    }
    else if (typescript.isTypeAliasDeclaration(node)) {
        return context.factory.updateTypeAliasDeclaration(node, node.decorators, newModifiers, node.name, node.typeParameters, node.type);
    }
            ...
            if (typescript.isObjectLiteralExpression(right)) {
                ...
                for (const property of right.properties) {
                    ...
                    // If it is a MethodDeclaration that can be safely rewritten to a function, do so
                        ...
                        context.addLocal(propertyName);
                        elements.push(factory.createShorthandPropertyAssignment(propertyName));
                        statements.push(factory.createFunctionDeclaration(property.decorators, addExportModifier(property.modifiers, context), property.asteriskToken, property.name, property.typeParameters, property.parameters, property.type, property.body));
                    }
            ...
            if (!context.isLocalExported(exportsData.property)) {
                ...
                if (typescript.isIdentifier(continuationResult)) {
                    ...
                    if (freeIdentifier === exportsData.property) {
                        ...
                        context.addTrailingStatements(factory.createVariableStatement(undefined, factory.createVariableDeclarationList([factory.createVariableDeclaration(freeIdentifier, undefined, undefined, continuationResult)], typescript.NodeFlags.Const)), factory.createExportDeclaration(undefined, undefined, false, namedExports, undefined));
                    }
                }
            }
            return undefined;
...
function visitVariableDeclaration({ node, childContinuation, sourceFile, context }) {
    ...
    if (typescript.isIdentifier(node.name)) {
        ...
        // depending on whether or not the module has a default export
            ...
            context.addImport(factory.createImportDeclaration(undefined, undefined, moduleExports == null || moduleExports.hasDefaultExport
                ? // Import the default if it has any (or if we don't know if it has)
    ...
    // We will only consider the simplest variants of these before opting out and letting the CallExpression visitor handle more sophisticated behavior
        ...
        if (importSpecifiers.length > 0) {
            ...
            for (const importSpecifier of importSpecifiersThatWillBeReassigned) {
                ...
                context.addImport(factory.createImportDeclaration(undefined, undefined, factory.createImportClause(false, undefined, namedImports), factory.createStringLiteral(transformedModuleSpecifier), maybeGenerateAssertClause(context, transformedModuleSpecifier, moduleExports === null || moduleExports === void 0 ? void 0 : moduleExports.assert)), moduleSpecifier);
                // Now, immediately add a local mutable variable with the correct name
...
function transformSourceFile(sourceFile, context) {
    ...
    for (const statement of updatedSourceFile.statements) {
        ...
        else if (statement.modifiers != null && statement.modifiers.some((m) => m.kind === typescript.SyntaxKind.ExportKeyword)) {
...
function cjsToEsmTransformer(options = {}) {
    return context => {
        ...
        const visitorContext = {
            ...
            factory: (_a = context.factory) !== null && _a !== void 0 ? _a : typescript,
...
export { cjsToEsm, cjsToEsmTransformer };
